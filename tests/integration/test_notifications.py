...
def test_get_notifications_empty():
    ...
    # Get notifications
    response = client.get("/users/me/notifications", cookies=login_response.cookies)
...
def test_get_notifications_after_follow():
    ...
    # Alice follows bob
    follow_response = client.post("/users/user_002/follow", cookies=alice_login.cookies)
...
def test_get_notifications_multiple_follows():
    ...
    response = client.get("/users/me/notifications", cookies=charlie_login.cookies)
    assert response.status_code == 200
...
def test_get_notifications_pagination():
    ...
    # Get first 2
    response1 = client.get("/users/me/notifications?limit=2&offset=0", cookies=target_login.cookies)
    ...
    response2 = client.get("/users/me/notifications?limit=2&offset=2", cookies=target_login.cookies)
...
def test_notifications_persist_after_reload(tmp_path):
    ...
    temp_dao.email_index = {'alice@test.com': 'user_001', 'bob@test.com': 'user_002'}
...
def test_notifications_invalid_session():
    ...
    response = client.get("/users/me/notifications", cookies={"session": "invalid_token"})
    assert response.status_code == 401
