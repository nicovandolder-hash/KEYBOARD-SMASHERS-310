...
def create_and_login_user(client, username, email, password):
    ...
    assert register_response.status_code == 201, f"Failed to register {username}"
...
def test_get_followers(client):
    ...
    charlie_id, charlie_token = create_and_login_user(client, "charlie", "charlie@example.com", "CharliePass123!")
    
    # Bob and Charlie follow Alice
    client.post(f"/users/{alice_id}/follow", cookies={"session_token": bob_token})
    client.post(f"/users/{alice_id}/follow", cookies={"session_token": charlie_token})
...
def test_followers_pagination(client):
    ...
    for i in range(3):
        ...
        client.post(f"/users/{alice_id}/follow", cookies={"session_token": token})
