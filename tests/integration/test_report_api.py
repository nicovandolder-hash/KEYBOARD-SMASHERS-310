...
def test_data_files():
    ...
    original_reviews_path = review_controller_instance.review_dao.new_reviews_csv_path
    ...
    review_controller_instance.review_dao.new_reviews_csv_path = reviews_file.name
    ...
    review_controller_instance.review_dao.new_reviews_csv_path = original_reviews_path
    review_controller_instance.report_dao.csv_path = original_reports_path
    review_controller_instance.report_dao.load_reports()

    # Cleanup temp files
    for path in [users_file.name, movies_file.name, reviews_file.name, reports_file.name]:
        try:
            Path(path).unlink()
        except Exception:
            pass
...
class TestReportingIntegration:
    ...
    def test_full_reporting_workflow(self, client, test_data_files):
        """Test complete workflow: create users, login, create review, report it."""
    ...
    def test_cannot_report_twice(self, client, test_data_files):
        ...
        response = client.post(
            "/reviews/",
            json={"movie_id": "movie_001", "rating": 3.0, "review_text": "Okay"},
    ...
    def test_multiple_users_can_report_same_review(self, client, test_data_files):
        """Test that multiple different users can report the same review."""
        ...
        response = client.post(
            "/reviews/",
            json={"movie_id": "movie_001", "rating": 2.0, "review_text": "Bad"},
            cookies={"session_token": user1_session}
        )
        review_id = response.json()["review_id"]
        ...
        with open(test_data_files['reports'], 'r') as f:
            ...
            review_reports = [r for r in reports if r['review_id'] == review_id]
