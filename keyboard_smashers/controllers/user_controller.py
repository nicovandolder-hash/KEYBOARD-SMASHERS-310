...
class UserAPISchema(BaseModel):
    ...
    username: str = Field(..., description="User's display name", example="reviewer_bob")
    email: str = Field(..., description="User's email address", example="bob@stu.ubc.ca")
    reputation: int = Field(3, description="User reputation score")
    creation_date: datetime = Field(None, description="Date user account was created")
    total_reviews: int = Field(0, description="Total number of reviews written")
...
class UpdateUserSchema(BaseModel):
    ...
    reputation: Optional[int] = Field(None, description="User reputation score")
    is_admin: Optional[bool] = Field(None, description="Whether the user is an administrator")
...
class UserController:
    ...
    def load_users(self, csv_path: str):
        try:
            ...
            with open(csv_path, 'r', encoding = 'utf-8') as f:
                ...
                for row in reader:
                    creation_date = datetime.fromisoformat(row['creation_date']) if row.get('creation_date')  else datetime.now()
                    ...
                    if user.userid.startswith("user_"):
                        try:
                            ...
                            self._user_counter = max(self._user_counter, user_num + 1)
    ...
    def save_users_to_csv(self, csv_path: str):
        try:
            ...
            with open(csv_file, 'w', encoding='utf-8', newline='') as f:
                fieldnames = ['userid', 'username', 'email', 'password', 'reputation', 
    ...
    def authenticate_user(self, email: str, password: str):
        ...
        logger.info(f"User authenticated successfully: {user.username} ({user.userid})")
        return user
    ...
    def update_user_by_id(self, user_id: str, user_data: UpdateUserSchema) -> UserAPISchema:
        ...
        try:
            if user_data.username is not None:
                logger.debug(f"Updating username from: {user.username} --> {user_data.username}")
                user.username = user_data.username
            if user_data.email is not None:
                new_email_lower = user_data.email.lower()  
                if any(u.email.lower() == new_email_lower and u.userid != user_id for u in self.users):
                    logger.warning(f"Email update failed for user {user_id}: Email already registered")
                    raise HTTPException(status_code=400, detail="Email already registered")
                ...
                logger.debug(f"Updating email from: {user.email} --> {user_data.email}")
            ...
            if user_data.reputation is not None:
                logger.debug(f"Updating reputation from: {user.reputation} --> {user_data.reputation}")
                user.reputation = user_data.reputation
            if user_data.is_admin is not None:
                logger.debug(f"Updating is_admin from: {user.is_admin} --> {user_data.is_admin}")
...
@router.post("/logout")
def logout(response: Response, session_token: Optional[str] = Cookie(default=None, alias="session_token")):
    from keyboard_smashers.auth import SessionManager
...
def get_user(
    ...
    if current_user_id != user_id and not current_user.is_admin:
        raise HTTPException(
            status_code=403,
            detail="You can only view your own profile. Admin privileges required to view other profiles."
        )
